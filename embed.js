(function(){
  ...
  var heading=customTitle||'Request a free quote';
  ...
  wrap.innerHTML='<div class="om-lw-card">'
    ...
    +'<div class="om-lw-sub" data-om-sub>Fill this out and we will get right back to you.</div></div></div>'
  ...
  try{fetch(API+'?c='+encodeURIComponent(acct)).then(function(r){return r.json();}).then(function(d){
    ...
    if(d.company){company=d.company;if(!customTitle){var hh=wrap.querySelector('[data-om-h]');if(hh)hh.textContent='Request a quote from '+d.company;}}
  ...
  form.addEventListener('submit',function(ev){
    ...
    fetch(API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(fd)}).then(function(r){return r.json();}).then(function(d){
      if(d&&d.ok){card.innerHTML='<div class="om-lw-ok"><div class="om-lw-check">\u2713</div><h4>Thanks, '+esc(fd.name.split(' ')[0])+'!</h4><p>Your request is in'+(company?(' with '+esc(company)):'')+' \u2014 we will be in touch shortly.</p></div><div class="om-lw-foot">Powered by <a href="https://orchamind.com" target="_blank" rel="noopener">Orchamind</a></div>';}

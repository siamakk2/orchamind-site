...
function page(title, bodyHtml){
  return '<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    ...
    + '.card{background:#13243d;border:1px solid #24395c;border-radius:16px;padding:32px;max-width:480px;}'
    + 'h2{margin:0 0 10px;}a{color:#2D7FF9;font-weight:700;text-decoration:none;}pre{white-space:pre-wrap;word-break:break-word;background:#0a1628;padding:12px;border-radius:8px;color:#9bb8e0;font-size:12px;}</style>'
    + '<div class="card"><h2>' + title + '</h2>' + bodyHtml + '</div>';
}
module.exports = async (req, res) => {
  res.setHeader('Content-Type','text/html');
  try {
    var u = new URL(req.url, 'https://orchamind.com');
    var code = u.searchParams.get('code');
    var realmId = u.searchParams.get('realmId') || '';
    if (!code) { res.statusCode = 400; return res.end(page('Connection cancelled','<p>No authorization code was returned. <a href="/api/qbo/connect">Try connecting again</a>.</p>')); }
    ...
    if (!tok || !tok.access_token) {
      res.statusCode = 200;
      return res.end(page('QuickBooks connection failed', '<p>Intuit did not return a token. Details:</p><pre>' + esc(JSON.stringify(tok)) + '</pre><p><a href="/api/qbo/connect">Try again</a></p>'));
    }
    ...
    if (SB_URL && SB_KEY) {
      ...
      storeErr = 'Supabase env vars not found.';
    }
    res.statusCode = 200;
    if (stored) {
      return res.end(page('\u2713 QuickBooks connected', '<p>Your QuickBooks company (realm <b>' + esc(realmId) + '</b>) is linked to Orchamind.</p><p><a href="/app">Back to Orchamind &rarr;</a></p>'));
    }
    return res.end(page('Connected, but not saved', '<p>Intuit authorized successfully, but storing the token failed:</p><pre>' + esc(storeErr) + '</pre><p>Tell your developer this message. Tokens were not saved.</p>'));
  } catch (e) {
    res.statusCode = 200;
